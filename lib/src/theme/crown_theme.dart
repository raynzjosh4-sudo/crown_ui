...
class CrownTheme extends InheritedWidget {
  ...
  const CrownTheme({Key? key, required this.data, required Widget child}) : super(key: key, child: child);

  static CrownThemeData of(BuildContext context) {
    final CrownTheme? result = context.dependOnInheritedWidgetOfExactType<CrownTheme>();
    assert(result != null, 'No CrownTheme found in context.');
    return result!.data;
  }
